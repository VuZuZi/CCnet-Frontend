    ...
    <div className="min-h-screen bg-[#f6f7fb] py-10 px-4">
      <div className="w-full max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          <div>
            <h1 className="text-[28px] font-extrabold tracking-tight text-[#111827] mb-1">Following</h1>
            <p className="text-sm text-[#6b7280] m-0">{items?.length || 0} accounts</p>
          </div>
        ...
        {isLoading && (
          <div className="bg-white rounded-[14px] shadow-sm flex justify-center p-7 border border-light-gray">
             <svg className="animate-spin h-6 w-6 text-yellow" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-1 gap-3">
            {filtered.length === 0 ? (
              <div className="bg-white rounded-[14px] shadow-sm p-7 text-center border border-light-gray">
                ...
                <div className="text-[#6b7280] mt-1.5 text-sm">Try another keyword or follow someone first.</div>
              </div>
            ) : (
              filtered.map((u) => {
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-11 h-11 rounded-full overflow-hidden shrink-0 bg-[#f3f4f6] flex items-center justify-center font-black text-[#111827]">
                        {u.avatar ? (
                          <img className="w-full h-full object-cover" src={u.avatar} alt={title} />
                        ) : (
                          letter
                        )}
                      </div>
                      ...
                      <div className="min-w-0">
                        <div className="font-extrabold text-[#111827] truncate leading-tight mb-0.5">{title}</div>
                        <div className="text-[13px] text-[#6b7280] truncate">{u.email || ''}</div>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-light-gray flex justify-between items-center bg-white">
              ...
              {!unfollowMutation.isPending && (
                <button 
                  onClick={() => { setConfirmOpen(false); setPendingUser(null); }}
