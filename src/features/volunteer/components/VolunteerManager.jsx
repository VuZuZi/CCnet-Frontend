...
export const VolunteerManager = ({ projectId, initialSubTab = 'pending' }) => {
    ...
    // CreatePostPage Update state when prop changes
    useEffect(() => {
        if (initialSubTab) {
    ...
    // CreatePostPage Lấy dữ liệu cho 3 status
    ...
    // CreatePostPage Chọn dữ liệu theo tab
    ...
    // CreatePostPage Approve: PENDING -> APPROVED
    const handleApprove = (applicationId) => {
        ...
        if (confirm('Xác nhận duyệt đơn đăng ký này?')) {
            ...
            approveApplication({ id: applicationId}, {
                onSuccess: () => {
                    console.log('CreatePostPage Approve success');
                    refetchPending();
                    refetchApproved();
    ...
    // CreatePostPage Reject: PENDING -> REJECTED
    ...
    const handleRestore = (applicationId) => {
        if (confirm('Xác nhận khôi phục đơn đăng ký này?')) {
            // CreatePostPage Đúng format: { id, data: { status: 'PENDING' } }
            restoreApplication(
