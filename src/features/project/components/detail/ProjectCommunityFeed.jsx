...
export function ProjectCommunityFeed({ project, isOrganizer }) {
  ...
  }, [application?.status, isVolunteerRole]);

  const canPost = useMemo(() => {
    if (!user?.id) return false;
    if (isOrganizer) return true;
    return isApprovedVolunteer;
  }, [isApprovedVolunteer, isOrganizer, user?.id]);
  ...
  const handleCreatePost = async () => {
    ...
    try {
      ...
      const apiError =
        error?.response?.data?.message || error?.message || "Có lỗi khi đăng bài";
  ...
  const handleCreateComment = async (postId) => {
    if (!canEngage) {
      toast.error("Chỉ project owner hoặc volunteer đã được duyệt mới có thể bình luận.");
      return;
    }
    ...
    try {
      ...
      const apiError =
        error?.response?.data?.message || error?.message || "Có lỗi khi bình luận";
      toast.error(apiError);
    }
  };
              ...
              <div className="flex items-center justify-end gap-2 sm:gap-3">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-colors ${
                    !canPost || (!postContent.trim() && !postMedia) || createPost.isPending
                      ? "bg-slate-200 text-slate-500 cursor-not-allowed"
        ...
        >
          ...
          {Array.isArray(p.media) && p.media.length > 0 && (
            <div className="rounded-2xl overflow-hidden bg-slate-100 aspect-video">
              {p.media[0].type?.startsWith("video") || p.media[0].url?.match(/\.(mp4|webm|mov)$/i) ? (
                <video className="w-full h-full object-cover" src={p.media[0].url} controls />
              ) : (
                <img
                  alt="post media"
          ...
          <div className="flex items-center gap-6 pt-2 border-t border-slate-100">
            <button
              ...
              onClick={() => {
                if (!canEngage) {
                  toast.error("Chỉ project owner hoặc volunteer đã được duyệt mới có thể thả tim.");
                  return;
                }
                togglePostLike.mutate(p._id);
              }}
              disabled={togglePostLike.isPending}
              className={`flex items-center gap-1.5 text-sm font-medium transition-colors ${
                p.likedByMe ? "text-red-500" : "text-slate-500 hover:text-red-500"
              }`}
          ...
          <div className="bg-slate-50 p-4 rounded-2xl space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-3 mb-1">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        toggleCommentLike.mutate(c._id);
                      }}
                      disabled={toggleCommentLike.isPending}
                      className={`flex items-center gap-1 text-xs font-bold ${
                        c.likedByMe ? "text-red-500" : "text-slate-400 hover:text-red-500"
                      }`}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <div className="flex-1 flex items-center gap-2">
                <input
                  className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary disabled:bg-slate-100"
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors ${
                    !canEngage || !String(commentDrafts[p._id] || "").trim() || createComment.isPending
                      ? "bg-slate-200 text-slate-500 cursor-not-allowed"
