        ...
        <div className="space-y-8 p-2 sm:p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-[24px] bg-emerald-50 border border-emerald-100 p-6">
                    ...
                    <p className="text-3xl font-black text-slate-900">{totalDonors.toLocaleString()} <span className="text-lg font-medium text-slate-500">lượt</span></p>
                </div>
                <div className="rounded-[24px] bg-amber-50 border border-amber-100 p-6">
                    ...
                    <p className="text-3xl font-black text-slate-900">{(project?.financialDetail?.availableBalance || 0).toLocaleString()} <span className="text-lg font-medium text-slate-500">đ</span></p>
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                {donors.length === 0 ? (
                    <div className="py-20 text-center">
                        ...
                        <p className="text-slate-500 font-medium">Chưa có dữ liệu đóng góp công khai cho dự án này.</p>
                    </div>
                ) : (
                    <div className="divide-y divide-slate-50 border border-slate-100 rounded-[32px] overflow-hidden bg-white">
                        {donors.map((tx) => {
                            ...
                            return (
                                <div key={tx._id} className={`flex items-center justify-between p-5 transition-colors ${isReversed ? 'bg-slate-50/50 opacity-70' : 'hover:bg-slate-50/50'}`}>
                                    <div className="flex items-center gap-4">
                                        <div className="h-12 w-12 rounded-full border-2 border-slate-100 overflow-hidden bg-slate-50 flex items-center justify-center">
                                            {tx.donorRef?.avatar ? (
                                                <img src={tx.donorRef.avatar} alt="avatar" className="h-full w-full object-cover" />
                                            ) : (
                                                <UserCircle2 className="text-slate-300" size={32} />
                                            )}
                                        </div>
                                        <div>
                                            <p className={`font-bold ${isReversed ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className={`text-[10px] font-bold uppercase tracking-tighter ${isReversed ? 'text-rose-500' : 'text-slate-400'}`}>
