...
export function BudgetBreakdownBlock({ control, errors }) {
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm animate-in fade-in duration-300">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Giải trình chi tiết các khoản chi để đạt được mục tiêu gọi vốn.</p>
                </div>
            ...
            <div className="space-y-3">
                {fields.map((field, idx) => (
                    <div key={field.id} className="flex flex-col gap-1 p-3 border border-slate-100 rounded-xl bg-slate-50/50 hover:bg-slate-50 transition-colors">
                        <div className="flex flex-col md:flex-row gap-3 items-start md:items-center">
                            <div className="w-full md:flex-1">
                                <input
                                    {...control.register(`budgetBreakdown.${idx}.item`)}
                                    placeholder="Tên vật tư / khoản chi"
                                    className={cn(
                                        "w-full rounded-lg p-2.5 border bg-white text-sm outline-none transition-colors",
                                        errors.budgetBreakdown?.[idx]?.item ? 'border-red-500 focus:border-red-500' : 'border-slate-200 focus:border-[#fbbf24]'
                                    )}
                                />
                            </div>
                            ...
                            <div className="w-full md:flex-1">
                                <input
                                    {...control.register(`budgetBreakdown.${idx}.note`)}
                        ...
                        {errors.budgetBreakdown?.[idx]?.item && (
                            <span className="text-xs text-red-500 font-medium px-1">{errors.budgetBreakdown[idx].item.message}</span>
                        )}
                    </div>
                ))}
                ...
                {fields.length === 0 && (
                    <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-xl">
                        <p className="text-slate-500 text-sm">Chưa có khoản chi nào. Hãy thêm khoản chi hoặc sử dụng gợi ý mẫu.</p>
                    </div>
                )}
            </div>
