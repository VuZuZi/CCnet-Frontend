...
export default function Step1Story() {
  ...
  const processPayload = (data) => {
    ...
    const safeBeneficiary = data.beneficiaryInfo?.details
      ? DOMPurify.sanitize(data.beneficiaryInfo.details)
      : "";
  ...
  const onInvalid = (validationErrors) => {
    devConfig.log("Form Validation Failed:", validationErrors);
  ...
  return (
    <form onSubmit={handleNextStep} className="pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <fieldset
        disabled={isPending}
        className="grid grid-cols-1 lg:grid-cols-12 gap-8 group transition-opacity duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6">
            ...
            <div>
              ...
              />
              {errors.title && (
                <p className="text-red-500 text-sm mt-1.5 font-medium">{errors.title.message}</p>
              )}
            </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-4">
            ...
            <p className="text-sm text-slate-500">
              Upload quotes, permits, or local confirmation documents to increase credibility.
            </p>
            <Controller
              name="documents"
              control={control}
            ...
            />
            {errors.documents && (
              <p className="text-red-500 text-sm mt-1.5 font-medium">{errors.documents.message}</p>
            )}
          </div>
        </div>
      </fieldset>
          ...
          <button
            ...
            className={`w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-3 font-bold rounded-xl transition-all shadow-sm ${
              isPending
                ...
                : "bg-primary hover:bg-primary-hover text-white shadow-lg shadow-yellow-500/20"
            }`}
          >
            ...
            <div className="mt-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setDraftMeta((prev) => ({ ...prev, title: e.target.value }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraftMeta((prev) => ({ ...prev, projectType: e.target.value }))
                  }
                  className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm text-slate-700 outline-none focus:border-amber-300 focus:ring-2 focus:ring-amber-100"
                >
