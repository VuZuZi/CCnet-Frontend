        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="space-y-6">
                {fields.map((field, idx) => (
                    <div key={field.id} className="p-5 md:p-6 border border-slate-200 rounded-2xl bg-white shadow-sm relative group hover:border-[#fbbf24]/50 transition-colors">
                        <button
                            type="button"
                            onClick={() => remove(idx)}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pr-8">
                            <div className="md:col-span-2">
                                <label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-1 block">Tên giai đoạn {idx + 1}</label>
                                <input
                                    {...control.register(`milestones.${idx}.title`)}
                                    className={cn("w-full rounded-xl p-3 border outline-none transition-colors", errors.milestones?.[idx]?.title ? 'border-red-500' : 'border-slate-200 focus:border-[#fbbf24] bg-slate-50 focus:bg-white')}
                                    placeholder="VD: Khởi công & Đổ móng"
                                />
                                {errors.milestones?.[idx]?.title && <p className="text-red-500 text-xs mt-1 font-medium">{errors.milestones[idx].title.message}</p>}
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-1 block">Mô tả công việc</label>
                                <textarea
                                    {...control.register(`milestones.${idx}.description`)}
                                    className={cn("w-full rounded-xl p-3 border outline-none min-h-[80px] resize-y transition-colors", errors.milestones?.[idx]?.description ? 'border-red-500' : 'border-slate-200 focus:border-[#fbbf24] bg-slate-50 focus:bg-white')}
                                    placeholder="Mô tả chi tiết các hoạt động diễn ra trong giai đoạn này..."
                                />
                                {errors.milestones?.[idx]?.description && <p className="text-red-500 text-xs mt-1 font-medium">{errors.milestones[idx].description.message}</p>}
                            </div>
                            ...
                            <div className="md:col-span-2">
                                <label className="text-xs font-bold text-slate-600 uppercase tracking-wider mb-1 block">Kết quả bàn giao (Deliverables)</label>
                                <input
                                    {...control.register(`milestones.${idx}.deliverables`)}
                                    className={cn("w-full rounded-xl p-3 border outline-none transition-colors", errors.milestones?.[idx]?.deliverables ? 'border-red-500' : 'border-slate-200 focus:border-[#fbbf24] bg-slate-50 focus:bg-white')}
                                    placeholder="VD: Biên bản nghiệm thu, Hình ảnh thực tế móng cầu..."
                                />
                                {errors.milestones?.[idx]?.deliverables && <p className="text-red-500 text-xs mt-1 font-medium">{errors.milestones[idx].deliverables.message}</p>}
                            </div>
                        </div>
                    </div>
                ))}
