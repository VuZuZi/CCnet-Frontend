...
export const strictStep1Schema = (tierMaxDurationDays) => z.object({
  projectType: z.enum(PROJECT_TYPE),
...
export const strictStep2Schema = (isFunded, tierCapMaxFunding, projectStartDate, projectEndDate) => z.object({
  targetAmount: z.coerce.number().optional().default(0),
  mvpAmount: z.coerce.number().optional().default(0),
  budgetBreakdown: z.array(z.object({
  ...
  if (isFunded) {
    ...
    if (data.mvpAmount <= 0 || data.mvpAmount > data.targetAmount) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ngưỡng giải ngân MVP không hợp lệ', path: ['mvpAmount'] });
    }
    ...
    if (!data.milestones || data.milestones.length === 0) {
      ...
      data.milestones.forEach((m, idx) => {
        ...
        if (!m.startDate) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Bắt buộc', path: ['milestones', idx, 'startDate'] });
        if (!m.endDate) ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Bắt buộc', path: ['milestones', idx, 'endDate'] });

        if (m.startDate && m.endDate && new Date(m.startDate) >= new Date(m.endDate)) {
          ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Kết thúc sau bắt đầu', path: ['milestones', idx, 'endDate'] });
        }
        ...
        if (projectStartDate && projectEndDate) {
          ...
          if (m.startDate && new Date(m.startDate) < pStart) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ngoài TG dự án', path: ['milestones', idx, 'startDate'] });
          }
          if (m.endDate && new Date(m.endDate) > pEnd) {
            ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ngoài TG dự án', path: ['milestones', idx, 'endDate'] });
          }
        }
      });
    ...
    data.milestones?.forEach((m, idx) => {
      if (m.targetAmount > 0) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Dự án Volunteer không có ngân sách', path: ['milestones', idx, 'targetAmount'] });
      }
    });
  }
