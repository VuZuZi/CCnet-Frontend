        ...
        return (
          ...
          >
            <div className="mb-1 flex flex-wrap items-center gap-2">
              ...
              <span className={`flex-shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase ${urgencyStyle}`}>
                {item.urgencyLevel}
              </span>
            ...
            <p className="mb-3 line-clamp-1 text-sm text-slate-500">{item.story}</p>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              <div className="rounded-xl bg-slate-50 px-3 py-2">
                ...
                <p className="mt-0.5 truncate text-sm font-medium text-slate-700">{requesterName}</p>
              </div>
              <div className="rounded-xl bg-slate-50 px-3 py-2">
                <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                  <MapPin size={11} />
                ...
                <p className="mt-0.5 truncate text-sm text-slate-700">{item.location?.address || 'N/A'}</p>
              </div>
              <div className="rounded-xl bg-slate-50 px-3 py-2">
                <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <p className="mt-0.5 truncate text-sm font-semibold text-slate-700">
                  {item.amountNeeded ? formatCurrency(item.amountNeeded) : 'Flexible'}
                </p>
              </div>
              <div className="rounded-xl bg-slate-50 px-3 py-2">
                <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <p className="mt-0.5 truncate text-sm text-slate-700">{formatDate(item.createdAt) || 'Recently'}</p>
              </div>
            </div>
          </Link>
        );
