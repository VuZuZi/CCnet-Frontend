// src/shared/components/layouts/Navbar.jsx
...
import { showMessageToast } from '@/features/chat/components/ToastMessage';
...
export function Navbar() {
  ...
  // ✅ Lắng nghe tin nhắn mới để hiển thị toast
  useEffect(() => {
    if (!isAuthenticated) return;

    const handleNewMessage = (event) => {
      const { message, sender, conversationId } = event.detail;

      // Không hiển thị nếu là tin nhắn của chính mình
      if (sender?.id === user?.id) return;

      const senderName = sender?.fullName || sender?.name || 'Ai đó';
      const messageText = message?.text || message?.content || 'Đã gửi một tin nhắn';
      const senderAvatar = sender?.avatar;

      // ✅ Gọi showMessageToast từ ToastMessage
      showMessageToast(
          { name: senderName, avatar: senderAvatar },
          messageText,
          conversationId,
          openConversation
      );
  ...
  }, [isAuthenticated, user?.id, openConversation]);
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-20 items-center justify-between gap-4 lg:gap-8">
            ...
            <div className="flex flex-shrink-0 items-center space-x-2 sm:space-x-4">
              ...
              {isAuthenticated ? (
                  <>
                    <ChatAction unreadCount={unreadCount} />
                    <NotificationAction />
...
function ChatAction({ unreadCount }) {
